...
@section('content')
    <div class="space-y-5">
        ...
        <div class="bg-white/60 dark:bg-slate-800/60 backdrop-blur-xl border border-white/80 dark:border-slate-700/60 rounded-2xl px-5 py-4 shadow-sm">
            <form id="filterForm" action="{{ route('admin.kelas.index') }}" method="GET" class="flex flex-col sm:flex-row sm:items-center gap-2.5">
                ...
                <select name="unit_id" onchange="this.form.submit()"
                    class="h-[42px] px-2 sm:px-3 border-[1.5px] border-sky-100 rounded-[10px]
                           bg-sky-50 text-[11px] sm:text-[13px] text-slate-700 outline-none w-full sm:w-auto sm:min-w-[140px]
    ...
    <div id="bulkDeleteModal" class="fixed inset-0 z-[70] hidden items-center justify-center p-4 bg-slate-900/40 backdrop-blur-[2px] transition-opacity">
        <div class="bg-white dark:bg-slate-800 rounded-[2rem] p-8 w-full max-w-sm shadow-2xl border border-slate-200 dark:border-slate-700 transform transition-all text-center">
            ...
            <p class="text-sm text-slate-500 dark:text-slate-400 mb-8 leading-relaxed">
                Anda akan menghapus data yang dipilih. Tindakan ini tidak dapat dibatalkan. Kelas yang masih memiliki siswa tidak akan ikut terhapus.
            </p>
            <form id="bulkDeleteForm" action="{{ route('admin.kelas.bulk-destroy') }}" method="POST">
                @csrf
