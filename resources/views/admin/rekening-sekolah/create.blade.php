...
@section('content')
    <div class="space-y-5 max-w-3xl">
        ...
        {{-- PAGE HEADER --}}
        <div>
            <h1 class="text-2xl font-extrabold text-slate-800 tracking-tight mb-1">Tambah Rekening</h1>
            <p class="text-sm text-slate-500">Tambahkan rekening baru untuk pembayaran sekolah.</p>
        </div>

        {{-- FORM --}}
        <div class="bg-white border border-slate-100 rounded-2xl p-6 shadow-sm">
            <form action="{{ route('admin.rekening-sekolah.store') }}" method="POST" class="space-y-4">
                @csrf
                ...
                <div>
                    ...
                    <input type="text" name="account_number" value="{{ old('account_number') }}" required
                        class="w-full h-11 px-4 border border-slate-200 rounded-xl bg-slate-50 text-sm focus:bg-white focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 outline-none transition-all"
                        placeholder="Contoh: 71020108000184">
                ...
                <div>
                    ...
                    <input type="text" name="account_name" value="{{ old('account_name') }}" required
                        class="w-full h-11 px-4 border border-slate-200 rounded-xl bg-slate-50 text-sm focus:bg-white focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 outline-none transition-all"
                        placeholder="Contoh: Yayasan Wakaf Mutiara Qur'an">
                    @error('account_name') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                </div>

                <div class="flex items-center gap-2 mt-4">
                    <input type="checkbox" name="is_active" value="1" id="is_active" class="w-4 h-4 text-emerald-500 rounded border-slate-300 focus:ring-emerald-500" checked>
