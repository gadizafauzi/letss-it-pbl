...
@section('content')
    <div class="space-y-6">
        ...
        <div class="flex items-center gap-3 mb-2">
            ...
            <h1 class="text-xl font-bold text-slate-800 dark:text-slate-100">Edit Tahun Ajaran</h1>
        </div>
                ...
                <div class="xl:col-span-9">
                    <div class="bg-white/60 dark:bg-slate-800/60 backdrop-blur-xl border border-white/80 dark:border-slate-700/60 rounded-[2rem] shadow-sm p-6 space-y-6">
                        ...
                        <div>
                            ...
                            <input type="text" name="year" value="{{ old('year', $academicYear->year) }}" placeholder="Contoh: 2025/2026"
                        ...
                        <div>
                            ...
                            <select name="active_semester"
                                class="w-full h-12 px-4 rounded-2xl border bg-white/50 dark:bg-slate-900/50 border-sky-100 dark:border-slate-600 focus:bg-white focus:outline-none focus:ring-4 focus:ring-blue-100 focus:border-blue-400 text-sm text-slate-700 dark:text-slate-200 transition-all">
                                <option value="odd" {{ old('active_semester', $academicYear->active_semester) == 'odd' ? 'selected' : '' }}>Ganjil</option>
                                <option value="even" {{ old('active_semester', $academicYear->active_semester) == 'even' ? 'selected' : '' }}>Genap</option>
                            </select>
                        ...
                        <div class="border-t border-slate-100 dark:border-slate-700/50 pt-5">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                                <div>
                                    ...
                                    <input type="date" name="start_odd" value="{{ old('start_odd', optional($academicYear->start_odd)->format('Y-m-d')) }}"
                                        class="w-full h-12 px-4 rounded-2xl border bg-white/50 dark:bg-slate-900/50 border-sky-100 dark:border-slate-600 focus:bg-white focus:outline-none focus:ring-4 focus:ring-blue-100 focus:border-blue-400 text-sm text-slate-700 dark:text-slate-200 transition-all">
                                    @error('start_odd')
                                        <p class="text-xs text-red-500 mt-2">{{ $message }}</p>
                                    @enderror
                                ...
                                <div>
                                    ...
                                    <input type="date" name="end_odd" value="{{ old('end_odd', optional($academicYear->end_odd)->format('Y-m-d')) }}"
                                        class="w-full h-12 px-4 rounded-2xl border bg-white/50 dark:bg-slate-900/50 border-sky-100 dark:border-slate-600 focus:bg-white focus:outline-none focus:ring-4 focus:ring-blue-100 focus:border-blue-400 text-sm text-slate-700 dark:text-slate-200 transition-all">
                                    @error('end_odd')
                                        <p class="text-xs text-red-500 mt-2">{{ $message }}</p>
                                    @enderror
                        ...
                        <div class="border-t border-slate-100 dark:border-slate-700/50 pt-5">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                                <div>
                                    <label class="block text-sm text-slate-600 dark:text-slate-400 mb-2">Tanggal Mulai</label>
                                    <input type="date" name="start_even" value="{{ old('start_even', optional($academicYear->start_even)->format('Y-m-d')) }}"
                                        class="w-full h-12 px-4 rounded-2xl border bg-white/50 dark:bg-slate-900/50 border-sky-100 dark:border-slate-600 focus:bg-white focus:outline-none focus:ring-4 focus:ring-blue-100 focus:border-blue-400 text-sm text-slate-700 dark:text-slate-200 transition-all">
                                    @error('start_even')
                                        <p class="text-xs text-red-500 mt-2">{{ $message }}</p>
                                    @enderror
                                ...
                                <div>
                                    <label class="block text-sm text-slate-600 dark:text-slate-400 mb-2">Tanggal Selesai</label>
                                    <input type="date" name="end_even" value="{{ old('end_even', optional($academicYear->end_even)->format('Y-m-d')) }}"
                                        class="w-full h-12 px-4 rounded-2xl border bg-white/50 dark:bg-slate-900/50 border-sky-100 dark:border-slate-600 focus:bg-white focus:outline-none focus:ring-4 focus:ring-blue-100 focus:border-blue-400 text-sm text-slate-700 dark:text-slate-200 transition-all">
                                    @error('end_even')
                                        <p class="text-xs text-red-500 mt-2">{{ $message }}</p>
                                    @enderror
