    ...
    <section class="public-section bg-slate-50 relative overflow-hidden">
        ...
        <div class="w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-12 relative z-10">
            ...
            <div class="flex flex-nowrap md:flex-wrap justify-start md:justify-center gap-2 md:gap-3 mb-8 md:mb-12 overflow-x-auto pb-4 -mx-4 px-4 md:mx-0 md:px-0 custom-scrollbar snap-x">
                <button onclick="filterPrograms('all', this)" class="snap-start whitespace-nowrap flex-shrink-0 px-5 md:px-6 py-2 md:py-2.5 rounded-full text-xs md:text-sm font-bold bg-emerald-600 text-white shadow-md shadow-emerald-200 transition-all duration-500 filter-btn reveal reveal-up" style="transition-delay: 300ms;">Semua Program</button>
             ...
             <div class="grid grid-cols-2 lg:grid-cols-3 gap-3 md:gap-8">
                ...
                @foreach($displayPrograms as $idx => $p)
                    <div tabindex="0" class="bg-white rounded-2xl p-3 sm:p-6 min-h-[180px] md:min-h-[340px] h-full flex flex-col border border-slate-100 shadow-sm hover:shadow-[0_20px_40px_-12px_rgba(52,211,153,0.25)] hover:scale-[1.03] hover:-translate-y-2 transition-all duration-500 relative overflow-hidden group reveal reveal-program program-item {{ $animClasses[$idx % 6] }} focus:outline-none" data-category="{{ $p['category'] }}" style="transition-delay: {{ 120 + ($idx * 150) }}ms;">
                        ...
                        <div class="relative z-10 flex flex-col h-full transform transition-transform duration-500 group-hover:-translate-y-4 focus:-translate-y-4 flex-grow">
                            <div class="w-8 h-8 md:w-14 md:h-14 rounded-xl md:rounded-2xl bg-gradient-to-br from-emerald-50 to-emerald-100/60 text-emerald-600 flex items-center justify-center mb-2 md:mb-6 shadow-inner group-hover:-translate-y-1 group-hover:rotate-12 group-hover:scale-110 focus:-translate-y-1 focus:rotate-12 focus:scale-110 transition-transform duration-300">
                                <i data-lucide="{{ $p['icon'] }}" class="w-4 h-4 md:w-7 md:h-7"></i>
                            </div>
                            <h3 class="text-sm md:text-xl font-bold text-slate-800 mb-1.5 md:mb-4 group-hover:text-emerald-700 focus:text-emerald-700 transition-colors line-clamp-2">{{ $p['title'] }}</h3>
                            <div class="mt-auto pr-8 md:pr-0">
                                <span class="text-[9px] sm:text-[10px] md:text-xs font-bold text-emerald-600 bg-emerald-50 px-1.5 py-1 md:px-3 md:py-1.5 rounded-lg md:rounded-xl inline-block border border-emerald-100 line-clamp-2 leading-tight">
                                    {{ $p['detail'] }}
                                </span>
                            </div>
                        </div>
                        ...
                        <!-- Overlay Detail View -->
                        <div class="absolute inset-0 bg-gradient-to-t from-emerald-900 to-emerald-800/95 p-4 sm:p-6 md:p-8 flex flex-col justify-center overflow-y-auto translate-y-full group-hover:translate-y-0 focus:translate-y-0 transition-transform duration-500 ease-[cubic-bezier(0.2,0.8,0.2,1)] z-20 opacity-0 group-hover:opacity-100 focus:opacity-100 custom-scrollbar">
                            ...
                            <p class="text-emerald-50 text-xs md:text-sm leading-relaxed transform translate-y-8 group-hover:translate-y-0 focus:translate-y-0 transition-transform duration-500 delay-200 mb-auto line-clamp-3 md:line-clamp-none">{{ $p['desc'] }}</p>
                        </div>
                    </div>
                @endforeach
            </div>
